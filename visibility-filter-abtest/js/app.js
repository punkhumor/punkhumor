...
function allKnownVotes() {
  return mergeVotes(remoteResults.votes, getVotes());
}

async function refreshRemoteResults({ silent = false } = {}) {
  if (location.protocol === "file:") {
    remoteResults.status = "error";
    remoteResults.error = "本地打开无法同步，请用在线链接";
    if (state.view === "results") paint();
    return false;
  }
  remoteResults.status = "loading";
  if (!silent && state.view === "results") paint();
  try {
    const [{ votes }, tallies] = await Promise.all([
      fetchRemoteVotes(),
      fetchRemoteTallies(),
    ]);
    remoteResults.votes = votes;
    remoteResults.tallies = tallies;
    remoteResults.status = "ok";
    remoteResults.error = null;
    remoteResults.loadedAt = Date.now();
    if (state.view === "results") paint();
    return true;
  } catch (err) {
    remoteResults.status = "error";
    remoteResults.error = err?.message || "同步失败";
    if (state.view === "results") paint();
    return false;
  }
}
...
function renderResults() {
  ...
  const remote = remoteResults.votes || [];
  const votes = allKnownVotes();
  const preferA = votes.filter((v) => v.prefer === "A").length;
  const preferB = votes.filter((v) => v.prefer === "B").length;
  const tallyA = remoteResults.tallies?.A;
  const tallyB = remoteResults.tallies?.B;

  let statusLine = "";
  if (remoteResults.status === "loading") {
    statusLine = `<p class="lead">正在拉取全网投票…</p>`;
  } else if (remoteResults.status === "error") {
    statusLine = `<p class="lead">远程同步失败：${
      remoteResults.error || "未知错误"
    }。下面仍显示本机记录。</p>`;
  } else if (remoteResults.status === "ok") {
    const when = remoteResults.loadedAt
      ? new Date(remoteResults.loadedAt).toLocaleTimeString()
      : "";
    statusLine = `<p class="lead">已同步全网 <strong>${remote.length}</strong> 票${
      when ? `（${when}）` : ""
    }；本机另有 ${local.length} 条缓存。</p>`;
  } else {
    statusLine = `<p class="lead">打开本页会自动拉取其他人的投票。</p>`;
  }

  const tallyNote =
    tallyA != null || tallyB != null
      ? `<p class="muted" style="margin-top:8px">备用计数：A ${
          tallyA ?? "—"
        } · B ${tallyB ?? "—"}</p>`
      : "";
  ...
  const list = votes.length
    ? votes
        ...
        .reverse()
        .map((v) => {
          const mine = local.some((l) => l.id && l.id === v.id);
          return `<div class="result-card">
          <div><strong>投给方案 ${v.prefer}</strong> · ${new Date(
            v.at
          ).toLocaleString()}${mine ? " · 本机" : ""}</div>
        </div>`;
        })
        .join("")
    : `<p class="lead">还没有投票记录。</p>`;

  return `<section class="panel panel-pad">
    <h2 class="section-title">投票汇总</h2>
    ${statusLine}
    ...
    <div class="btn-row" style="margin-top:20px">
      <button class="btn btn-primary" type="button" data-act="refresh-votes">刷新全网</button>
...
function paint() {
  ...
  if (state.view === "intro") main.innerHTML = renderIntro();
    ...
    if (remoteResults.status === "idle" || remoteResults.status === "error") {
      refreshRemoteResults({ silent: remoteResults.status === "error" });
    }
  }
}
...
async function submitVote(form) {
  ...
  if (location.protocol !== "file:") {
    try {
      const remoteVotes = await appendRemoteVote(vote);
      remoteResults.votes = remoteVotes;
      remoteResults.status = "ok";
      remoteResults.error = null;
      remoteResults.loadedAt = Date.now();
      synced = true;
      try {
        const count = await bumpRemoteTally(prefer);
        if (prefer === "B") remoteResults.tallies.B = count;
        else remoteResults.tallies.A = count;
      } catch {
        /* tally is secondary */
      }
    } catch {
...
时间: ${new Date(vote.at).toLocaleString()}`;
  ...
  if (synced) {
    showToast(emailed ? "已提交到全网汇总" : "已写入全网汇总");
  } else if (emailed) {
    showToast("已发邮件（全网同步失败）");
  } else {
    try {
    ...
    a.click();
    showToast("同步失败，已复制并打开邮件");
  }
  state.view = "results";
  paint();
  if (!synced) refreshRemoteResults({ silent: true });
}
...
function wire() {
  ...
  root.addEventListener("click", (e) => {
    ...
    if (nav) {
      ...
      state.infoSheet = false;
      paint();
      if (state.view === "results") refreshRemoteResults({ silent: true });
      return;
    }
    ...
    if (e.target.closest("[data-act='export']")) {
      const blob = new Blob([JSON.stringify(allKnownVotes(), null, 2)], {
    ...
    if (e.target.closest("[data-act='refresh-votes']")) {
      refreshRemoteResults();
      return;
    }

    if (e.target.closest("[data-act='clear-votes']")) {
      if (confirm("清除本机投票缓存？不会删除全网已同步的票。")) {
